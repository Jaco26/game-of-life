...
function makeUniverse({ width, height, cols, rows } = {}) {
  ...
  function onClick(e) {
    ...
    console.log('onClick > cellIdx', cellIdx)
  ...
  function drawChangedCells() {
    for (let i = 0; i < state.changedCellIndices.length; i++) {
      ...
      ctx.fillStyle = cellValue === 1 ? '#333' : '#ffe'
...
document.querySelector('#btn-tick').addEventListener('click', function(e) {
  this.textContent = this.textContent === 'Tick' ? 'Tock' : 'Tick'
  universe.tick()
})
